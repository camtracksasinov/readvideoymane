CREATE OR REPLACE FUNCTION public.sp_weeklydrivedetails(vehicle_id integer,driver_id integer, start_date date, end_date date)
 
RETURNS TABLE(exceptiontype character varying, levellabel text,totalduration numeric,totaldistance numeric,
threshold numeric, timeexceeded numeric,startgps character varying,endgps character varying,
 clientname character varying, affiliatename character varying,transportername character varying,
...
coalesce(e.totaldistance,0) as totaldistance,coalesce(e.threshold,0) as threshold,coalesce(e.timeexceeded,0) as timeexceeded,
e.startposition as startgps,e.endpostion as endgps, cust.name as clientname,c.name as affiliatename,t.name as transportername,
...
and e.vehicleid=vehicle_id and e.driverid=driver_id and e.exceptiontype='9' 
and DATE(e.enddatetime::timestamp at time zone c.afftimezone) between start_date and end_date
order by e.enddatetime::timestamp at time zone c.afftimezone asc;
